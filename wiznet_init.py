...
def _pin(x):
    return x if isinstance(x, Pin) else Pin(x)
...
def wiznet(board, *, dhcp=True, spi=None, cs=None, reset=None, **kw):
    ...
    if spi is not None:
        ...
        if board in _AUTO:
            ...
            for k in ["sck", "cs", "io0", "io1", "io2", "io3"]:
                if k not in cfg:
                    raise ValueError("Missing pin '{}' for W6300 QSPI".format(k))
            spi = WIZNET_PIO_SPI(
                baudrate=cfg.get("baudrate", 31250000),
                sck=_pin(cfg["sck"]),
                cs=_pin(cfg["cs"]),
    ...
    if dhcp:
        ...
        sn = cfg.get("sn")
        gw = cfg.get("gw")
        dns = cfg.get("dns", gw or "8.8.8.8")
        if not (ip and sn and gw):
            raise ValueError("Static mode requires ip/sn/gw")
