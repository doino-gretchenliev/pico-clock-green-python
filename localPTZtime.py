...
def checkptz(ptz_string: str):
    ...
    if hasattr(re, "fullmatch"):  # re.fullmatch() is not defined in MicroPython
        ...
        check_re += (
            r"(?:,(?:J\d{1,3}|\d{1,3}|M(?:[1-9]|1[0-2])\.[1-5]\.[0-6])(?:\/\d{1,2}(?::\d{1,2}){0,2})?){2}"  # dst start/end date - time can be omitted
        )
...
def _normalize(ptz_string: str):
    ...
    ptz_string = re.compile(r"\<[^\>]*\>").sub(
        "DUMMY", ptz_string
    )  # For what appear to be non-standard strings like "<+11>-11<+12>,M10.1.0,M4.1.0/3"
